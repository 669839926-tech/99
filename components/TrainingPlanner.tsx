...
const TrainingPlanner: React.FC<TrainingPlannerProps> = ({ 
  ...
  const userManagedSessions = useMemo(() => {
      // 如果是总监，返回所有训练；如果是教练，仅返回其负责梯队的训练
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFormData({ teamId: availableTeams[0]?.id || '', title: '', focus: '传接球', focusCustom: '', duration: 90, intensity: 'Medium', date: new Date().toISOString().split('T')[0], drills: [], linkedDesignId: undefined });
            setIsAiMode(false);
