           ...
           <div id="player-profile-export" className="absolute left-[-9999px] top-0 w-[210mm] bg-white text-black p-0 z-[-1000] font-sans">
                   ...
                   <div className="flex-1 flex flex-col gap-5">
                               ...
                               {(['technical', 'tactical', 'physical', 'mental'] as AttributeCategory[]).map(cat => (
                                   <div key={cat} className="flex flex-col items-center">
                                       <div className="w-full h-[220px] relative border border-gray-100 rounded-xl p-2 bg-gray-50/30">
                                           ...
                                           <div className="absolute top-2 right-3 text-xl font-black text-gray-800">{getAvg(cat)}</div>
                                           <ResponsiveContainer width="100%" height="100%">
                                               <RadarChart cx="50%" cy="55%" outerRadius="70%" data={getCategoryRadarData(cat)}>
                                                   <PolarGrid stroke="#e5e7eb" />
                       ...
                       <div className="flex-1 flex flex-col min-h-0">
                           ...
                           <div className="grid grid-cols-2 gap-4 flex-1">
                               {editedPlayer.reviews
                                   ?.filter(r => r.year === exportYear)
                                   .sort((a,b) => a.quarter.localeCompare(b.quarter))
                                   .map((review, idx) => (
                                       <div key={review.id} className="bg-gray-50 p-4 rounded-lg border border-gray-200 text-xs h-full">
                               ...
                               {(!editedPlayer.reviews?.some(r => r.year === exportYear)) && (
                                   <div className="col-span-2 text-center text-gray-400 italic py-8 border border-dashed border-gray-300 rounded-lg">
                                       本年度暂无考评记录
                                   </div>
                               )}
                           </div>
               ...
               {editedPlayer.gallery && editedPlayer.gallery.some(p => p.date.startsWith(String(exportYear))) && (
                   <div className="w-full h-[297mm] p-[10mm] flex flex-col relative overflow-hidden bg-white break-before-page">
                       <div className="flex justify-between items-end border-b-4 border-bvb-yellow pb-4 mb-8">
                           <div>
                               <h1 className="text-2xl font-black uppercase tracking-tighter text-bvb-black">{editedPlayer.name}</h1>
                               <p className="text-sm font-bold text-gray-400 tracking-widest uppercase">年度精彩瞬间 / Gallery</p>
                           </div>
                           <div className="text-right">
                               <div className="text-3xl font-black text-bvb-yellow text-outline">{exportYear}</div>
                           </div>
                       </div>
...
const PlayerManager: React.FC<PlayerManagerProps> = ({ 
  ...
  const filteredPlayers = players.filter(p => {
    ...
    // New Birth Date Filter
    let matchesDate = true;
    if (birthDateStart) {
        matchesDate = matchesDate && (p.birthDate >= birthDateStart);
    }
    if (birthDateEnd) {
        matchesDate = matchesDate && (p.birthDate <= birthDateEnd);
    }

    return matchesTeam && matchesSearch && matchesPos && matchesDate;
  }).sort((a, b) => {
            ...
            {availableTeams.map(team => (
                <div key={team.id} className="relative group">
                    <button 
                        ...
                        className={`w-full text-left p-4 rounded-xl transition-all border-l-4 ${selectedTeamId === team.id ? 'bg-white border-