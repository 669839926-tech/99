...
const PlayerDetailModal: React.FC<PlayerDetailModalProps> = ({ 
    ...
    const renderRecords = () => {
        const events: any[] = []; (editedPlayer.rechargeHistory || []).forEach(r => events.push({ id: `rech-${r.id}`, originalId: r.id, date: r.date, type: 'recharge', amount: r.amount, desc: `充值 ${r.amount} 课时 (含请假额度 ${r.quotaAdded}次)`, quotaAdded: r.quotaAdded }));
                            ...
                            {editedPlayer.gallery.map(photo => (
                                <div key={photo.id} className="relative group bg-gray-100 rounded-xl overflow-hidden aspect-square border border-gray-200 shadow-sm flex flex-col">
                                    ...
                                    <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => { setEditingPhotoId(photo.id); setTempPhotoCaption(photo.caption || ''); }} className="p-1.5 bg-white/90 text-bvb-black rounded-full hover:bg-white shadow-sm" title="修改描述"><Edit2 className="w-3.5 h-3.5" /></button>
                                        <button onClick={() => handleDeletePhoto(photo.id)} className="p-1.5 bg-white/90 text-red-500 rounded-full hover:bg-white shadow-sm" title="删除"><Trash2 className="w-3.5 h-3.5" /></button>
                                    </div>
                                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-3 pt-10">
                                        {editingPhotoId === photo.id ? (
                                            <div className="flex items-center gap-2 animate-in fade-in duration-200">
                                                <input autoFocus className="flex-1 bg-white/20 border border-white/30 rounded px-2 py-1 text-xs text-white placeholder-white/50 focus:outline-none focus:ring-1 focus:ring-bvb-yellow" value={tempPhotoCaption} onChange={e => setTempPhotoCaption(e.target.value)} onBlur={() => handleSavePhotoCaption(photo.id)} onKeyDown={e => e.key === 'Enter' && handleSavePhotoCaption(photo.id)} placeholder="输入照片描述..." />
...
const PlayerManager: React.FC<PlayerManagerProps> = ({ 
  ...
  const handleAddPlayerSubmit = (e: React.FormEvent) => {
    ...
    if (newPlayer.name && newPlayer.name.trim() && finalTeamId && newPlayer.number !== undefined && !isNaN(newPlayer.number)) {
        ...
        const p: Player = { id: Date.now().toString(), teamId: finalTeamId, name: newPlayer.name.trim(), gender: newPlayer.gender || '男', idCard: newPlayer.idCard || '', birthDate: newPlayer.birthDate || '', number: newPlayer.number, position: (newPlayer.position || Position.ST) as Position, secondaryPosition: (newPlayer.secondaryPosition || Position.TBD) as Position, isCaptain: newPlayer.isCaptain || false, age: newPlayer.age || 16, goals: 0, assists: 0, appearances: 0, image: newPlayer.image || `https://picsum.photos/200/200?random=${Date.now()}`, stats: defaultStats, statsStatus: 'Published', lastPublishedStats: JSON.parse(JSON.stringify(defaultStats)), reviews: [], credits: 0, validUntil: nextYear.toISOString().split('T')[0], leaveQuota: 0, leavesUsed: 0, rechargeHistory: [], joinDate: newPlayer.joinDate || new Date().toISOString().split('T')[0], school: newPlayer.school || '', parentName: newPlayer.parentName || '', parentPhone: newPlayer.parentPhone || '', gallery: [], preferredFoot: (newPlayer.preferredFoot || '右') as '左' | '右', height: newPlayer.height, weight: newPlayer.weight, nickname: newPlayer.nickname || '' };
        onAddPlayer(p); setShowAddModal(false); setNewPlayer({ name: '', gender: '男', idCard: '', birthDate: '', age: 0, position: Position.ST, secondaryPosition: Position.TBD, number: 0, image: '', teamId: '', isCaptain: false, joinDate: '', school: '', parentName: '', parentPhone: '', preferredFoot: '右', height: undefined, weight: undefined, nickname: '' });
    } else {
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-100px)] md:h-auto gap-6 relative">
          ...
          <div className="bg-white w-full h-full md:h-auto md:max-w-2xl rounded-none md:rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col md:max-h-[90vh]">
            ...
            <form onSubmit={handleAddPlayerSubmit} className="p-6 overflow-y-auto flex-1 space-y-6 pb-24 md:pb-6">
                <div className="flex flex-col md:flex-row gap-6">
                    <div className="flex flex-col items-center space-y-3 shrink-0"><div className="relative group w-32 h-32 rounded-full overflow-hidden border-4 border-gray-100 bg-gray-50 flex items-center justify-center"><img src={newPlayer.image || `https://picsum.photos/200/200?random=${Date.now()}`} alt="Preview" className="w-full h-full object-cover" /><div onClick={() => fileInputRef.current?.click()} className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity"><Upload className="w-6 h-6 text-white mb-1" /><span className="text-[10px] text-white font-bold">上传照片</span></div><input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileChange} /></div><p className="text-[10px] text-gray-400 font-bold uppercase text-center">建议上传 1:1 照片</p></div>
                    <div className="flex-1 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                             <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">身份证号 (用于验证)</label><input className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none font-mono text-sm" placeholder="18位身份证号" maxLength={18} value={newPlayer.idCard} onChange={handleIdCardChange} /></div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">副位置 (可选)</label><PositionSelect value={newPlayer.secondaryPosition || Position.TBD} onChange={val => setNewPlayer({...newPlayer, secondaryPosition: val})} className="border-gray-200 border"/></div>
                        </div>
                    </div>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
                    <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">入队日期</label><input type="date" className="w-full p-2 border rounded text-xs font-bold" value={newPlayer.joinDate} onChange={e => setNewPlayer({...newPlayer, joinDate: e.target.value})}/></div>
                    <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">惯用脚</label><select className="w-full p-2 border rounded text-xs font-bold bg-white" value={newPlayer.preferredFoot} onChange={e => setNewPlayer({...newPlayer, preferredFoot: e.target.value as any})}><option value="右">右脚</option><option value="左">左脚</option></select></div>
                    <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">身高 (cm)</label><input type="number" className="w-full p-2 border rounded text-xs" value={newPlayer.height || ''} onChange={e => setNewPlayer({...newPlayer, height: parseInt(e.target.value)})} /></div>
                    <div><label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">体重 (kg)</label><input type="number" className="w-full p-2 border rounded text-xs" value={newPlayer.weight || ''} onChange={e => setNewPlayer({...newPlayer, weight: parseInt(e.target.value)})} /></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">家长姓名</label><input className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none text-sm" placeholder="家长姓名" value={newPlayer.parentName} onChange={e => setNewPlayer({...newPlayer, parentName: e.target.value})} /></div>
                    <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">联系电话</label><input className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none text-sm font-mono" placeholder="电话号码" value={newPlayer.parentPhone} onChange={e => setNewPlayer({...newPlayer, parentPhone: e.target.value})} /></div>
                </div>
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-center justify-between"><span className="text-sm font-bold text-yellow-800 flex items-center"><Crown className="w-4 h-4 mr-2" /> 赋予队长身份 (Captaincy)</span><label className="relative inline-flex items-center cursor-pointer"><input type="checkbox" className="sr-only peer" checked={newPlayer.isCaptain} onChange={(e) => setNewPlayer({...newPlayer, isCaptain: e.target.checked})}/><div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-bvb-yellow"></div></label></div>
                <button type="submit" className="w-full py-4 bg-bvb-black text-white font-bold rounded-lg hover:bg-gray-800 transition-colors shadow-lg flex items-center justify-center text-lg"><Save className="w-6 h-6 mr-2 text-bvb-yellow" /> 保存球员信息</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showAddTeamModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col">
                  <div className="bg-bvb-black p-4 flex justify-between items-center text-white shrink-0"><h3 className="font-bold flex items-center"><Shield className="w-5 h-5 mr-2 text-bvb-yellow" /> 新建梯队分组</h3><button onClick={() => setShowAddTeamModal(false)}><X className="w-5 h-5" /></button></div>
                  <form onSubmit={handleAddTeamSubmit} className="p-6 space-y-4">
                      <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">梯队名称</label><input required className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none font-bold" placeholder="例如: 2012 精英队" value={newTeam.name} onChange={e => setNewTeam({...newTeam, name: e.target.value})} /></div>
                      <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">年龄组别</label><input required className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none font-black" placeholder="例如: U11" value={newTeam.level} onChange={e => setNewTeam({...newTeam, level: e.target.value})} /></div>
                      <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">描述 (可选)</label><textarea className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none text-sm" placeholder="梯队简介..." value={newTeam.description} onChange={e => setNewTeam({...newTeam, description: e.target.value})} /></div>
      ...
      {showEditTeamModal && teamToEdit && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
              <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col">
                  <div className="bg-bvb-black p-4 flex justify-between items-center text-white shrink-0"><h3 className="font-bold flex items-center"><Edit2 className="w-5 h-5 mr-2 text-bvb-yellow" /> 编辑梯队信息</h3><button onClick={() => setShowEditTeamModal(false)}><X className="w-5 h-5" /></button></div>
                  <form onSubmit={handleUpdateTeamSubmit} className="p-6 space-y-4">
                      <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">梯队名称</label><input required className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none font-bold" value={teamToEdit.name} onChange={e => setTeamToEdit({...teamToEdit, name: e.target.value})} /></div>
                      <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">年龄组别</label><input required className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none font-black" value={teamToEdit.level} onChange={e => setTeamToEdit({...teamToEdit, level: e.target.value})} /></div>
                      <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">描述</label><textarea className="w-full p-2 border rounded focus:ring-2 focus:ring-bvb-yellow outline-none text-sm" value={teamToEdit.description} onChange={e => setTeamToEdit({...teamToEdit, description: e.target.value})} /></div>
